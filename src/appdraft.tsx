...
export const App = () => {
  ...
  const renumberNoteSteps = (
    ...
    return noteSteps.map((step, index) => ({
      ...step,
      text: ` ${step.text.split(": ")[1] || step.text}`,
    }));
  };
          ...
          for (const subIndex of index.subItems) {
            if (subIndex.checked) {
              await addNativeElement({
                type: "TEXT",
                children: [
                  `${currentReminders.indexOf(index) + 1}.${
                    index.subItems.indexOf(subIndex) + 1
                  }: ${subIndex.text}`,
                ],
              });
            }
  ...
  const addSubItem = (index: number) => {
    const updatedNoteSteps = noteSteps.map((step, i) =>
      i === index
        ? {
            ...step,
            subItems: [...step.subItems, { text: newNoteStep, checked: false }],
          }
        : step
    );
    setNoteSteps(updatedNoteSteps);
    setNewNoteStep(""); // Clear the input after adding the sub-item
  };
  ...
  const editSubItem = (noteIndex: number, subIndex: number) => {
    setNewNoteStep(noteSteps[noteIndex].subItems[subIndex].text);
    setEditingSubItemIndex({ noteIndex, subIndex });
  };
          ...
          <div className={styles.noteDetails}>
            ...
            <div className={styles.section}>
              ...
              <div className={styles.toolInputContainer}>
                ...
                />
                <Button variant="primary" onClick={addOtherTool}>
                  <FontAwesomeIcon icon={faPlus} />
                </Button>
              </div>
            ...
            <div className="list-section">
              ...
              />
              <div className={styles.footnote} style={{ marginBottom: "10px" }}>
              Press enter to create a section and click on the plus sign to add row notes
              </div>
                            ...
                            >
                              <div className={styles.stepItem}>
                                ...
                                <div className={styles.actions}>
                                  ...
                                  <div
                                    className={styles.iconButton}
                                    onClick={() => editNoteStep(index)}
                                  >
                                    <FontAwesomeIcon icon={faPen} />
                                  </div>
                                  <div
                                    className={styles.iconButton}
                                    onClick={() => removeNoteStep(index)}
                                  >
                                    <FontAwesomeIcon icon={faTimes} />
                                  </div>
                                </div>
